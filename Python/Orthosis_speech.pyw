...
class RTPlot(QMainWindow):
        ...
        def __init__(self, parent = None):
                ...
                loadUi('Orthosis_speech.ui', self)
		...
		self.pushButton_sitstand.clicked.connect(self.sitstand)
                self.pushButton_record.clicked.connect(self.record)
                ...
                #Security variables during the speech
                self.DANGER_FLEX_RIGHT=0
                self.DANGER_FLEX_LEFT=0
	...
	def getCommand(self, phrase, commands=commands):
	...
	def record(self):
 		if self.pushButton_record.isChecked():
			...
			try:
				...
				RESULT = self.getCommand(hypothesis.hypstr)
				...
				if(RESULT=="GO"):
				  ...
				  if(self.DANGER_FLEX_RIGHT==0 and self.DANGER_FLEX_LEFT==0):
                                          self.DANGER_FLEX_LEFT=1
				          self.DANGER_FLEX_RIGHT=1
				  ...
				  if (self.DANGER_FLEX_RIGHT==1 and self.DANGER_FLEX_LEFT==1):
				  ...
				  print("DOES NOT EXIST ACTION REQUIERED")

		        except:
				print "ABLE: Sometihng went wrong!"
	...
	def sitstand(self):
 		if self.pushButton_sitstand.isChecked():
			...
			if self.connected:
				self.pushButton_sitstand.setText("STAND UP")
				self.sendCommand(b"SITDOWN")
	...
	def flexionextension_right(self):
 		if self.pushButton_FlexExt_RIGHT.isChecked():
			...
			self.sendCommand(b"EXTENSIONRIGHT")
                ...
                if self.pushButton_FlexExt_RIGHT.isChecked() | self.pushButton_FlexExt_LEFT.isChecked():
		        self.startButton.setChecked(False)
		        self.startButton.setEnabled(False)
		        self.startButton.setText("Start")
